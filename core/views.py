...
def showCropStateWise(request):
    df=pd.read_csv("C:\\Users\\dhananjay\\sem7\\crop_production.csv")
    df.dropna(subset=["Production"],axis=0,inplace=True)
    opYear = df.Crop_Year.unique()
    opSeason = df.Season.unique()
    if request.method=="POST":
        s=request.POST["state"].strip()
        try:
            d=request.POST["district"].strip()
        except MultiValueDictKeyError:
            d="NA"
        y=request.POST["year"].strip()
        sea=request.POST["season"].strip()
        print("---------------------->",s,d,y,sea)
        ...
        for key in yearWiseCrop.keys():
            cropList = yearWiseCrop[key]
            cropPercent = {}
        ...
        print("--------------------->",crops)
        return render(request,"crop-year-state-wise.html",{"myData":myData,"crops":crops,"stateDef":s,"districtDef":d,"yearDef":y,"seasonDef":s,"years":opYear,"seasons":opSeason})
    # print(crops)
